    ...
    async function autoAnswer() {
        ...
        while (true) {
            if(features.autoAnswer && features.questionSpoof) {
                // Modificação para processar múltiplas unidades
                const units = document.querySelectorAll('.unit-header'); // Seleciona todas as unidades
                units.forEach(unit => {
                    ...
                    classToCheck.forEach(async (q) => {
                        ...
                        if(element && element.textContent=='Mostrar resumo') {
                            sendToast(" Unidade concluída!", 3000);
                            playAudio('https://r2.e-z.host/4d0a0bea-60f8-44d6-9e74-3032a64a9f32/4x5g14gj.wav');
                            
                            // Tenta abrir a próxima unidade
                            const nextUnit = unit.nextElementSibling;
                            if(nextUnit) nextUnit.click();
                        }
                    });
                });
            }
